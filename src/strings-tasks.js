...
function countVowels(str) {
  const dic = 'aeiouyAEIOUY';
  let count = 0;
  for (let i = 0; i < str.length; i += 1) {
    if (dic.includes(str[i])) count += 1;
  }
  return count;
}
...
function invertCase(str) {
  let res = '';
  for (let i = 0; i < str.length; i += 1) {
    if (str[i].toLowerCase() === str[i]) res += str[i].toUpperCase();
...
function encodeToRot13(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (dic.indexOf(str[i]) <= 12 && dic.indexOf(str[i]) !== -1) {
      ...
      dic.indexOf(str[i]) >= 13 &&
      dic.indexOf(str[i]) < 26 &&
      dic.indexOf(str[i]) !== -1
    ) {
      result += dic[dic.indexOf(str[i]) - 13];
    } else if (
      dic.indexOf(str[i]) >= 26 &&
      dic.indexOf(str[i]) < 39 &&
      dic.indexOf(str[i]) !== -1
    ) {
      result += dic[dic.indexOf(str[i]) + 13];
